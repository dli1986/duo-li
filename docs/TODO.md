...
| Knowledge | Pipeline rebuilt with `kind`/`sourceType` discriminator + subfolder layout (`docs/knowledge-architecture.md`). Only 2 top-level categories now: `AI`, `Systems Engineering` — everything finer-grained is a `tag`. **20 real entries** as of 2026-09-23: the original 6 (`ner-encoder-only`, `agent-architecture-state-graph-middleware`, `grpo-reasoning-vs-reward-shaping`, `cpp-copy-move-semantics-leveldb`, `aix-linux-cross-platform-eval-order-bug`, `azure-wireguard-vpn-runbook`) plus a full-folder migration of `PKG/C++ interview & Knowledge/` (12 new concept/article entries covering abstract classes, containers, virtual functions, LevelDB Slice semantics, class invariants, static/dynamic cast, class hierarchies, RAII/ownership, move semantics, templates/ADL, an interview-prep roadmap, and a buddy-allocator page-fault performance case study — see `docs/knowledge-architecture.md`'s "C++ interview & Knowledge full folder migration" section for the source-file mapping). `C++ interview & Knowledge/` is now **fully migrated**, nothing left unmigrated in that folder. |
