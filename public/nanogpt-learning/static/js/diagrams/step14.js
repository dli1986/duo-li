function drawStep14(ctx, canvasWidth, canvasHeight) {
  ...
  ctx.fillText("(65,64) preview, row " + tracedToken + " highlighted \u2014 FROZEN", tblX - 10, tblY - 4);
  drawMiniLookupTable(ctx, tblX, tblY, rows, cols, cw, ch, [tracedToken], 12345);
  ctx.fillStyle = "#3fb950"; ctx.font = "14px monospace";
  ctx.fillText("same table as steps 3 & 13", tblX - 10, tblY + rows * ch + 22);
  ctx.fillStyle = "#8b949e"; ctx.font = "12.5px monospace";
  ctx.fillText("this new token id (illustrative continuation)", tblX - 10, tblY + rows * ch + 42);
  ctx.fillText("same mechanism, no new lookup rule", tblX - 10, tblY + rows * ch + 58);

  const bW = 1, bH = 3, bD = 16, bScale = 40, bOX = 700, bOY = 185;
  ...
  renderTensorBox(ctx, canvasWidth, canvasHeight, {
    ...
    heatFn: (c, d, W_, D_) => {
      ...
      return divColor(EMB_W[TOKEN_GRID[0][0]][dim]);
    },
    title: "decode step: (B, 1, n_embd) \u2014 only ONE new column",
    seqLabel: "T=1", seqSubLabel: "",
  ...
  ctx.fillText("this box is the CACHE version\u2019s shape (see red note below) \u2014 the lookup mechanism itself is identical either way", 16, canvasHeight - 16);
}
